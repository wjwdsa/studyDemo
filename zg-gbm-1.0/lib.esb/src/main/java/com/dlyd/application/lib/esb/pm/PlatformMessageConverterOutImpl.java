...
public class PlatformMessageConverterOutImpl implements PlatformMessageConverter {

  private final XMLInputFactory xmlInputFactory;
  private final XMLOutputFactory xmlOutputFactory;

  public PlatformMessageConverterOutImpl() {
  ...
  public byte[] fromPlatformMessage(PlatformMessage platformMessage) {
    try {
      ...
      Optional<E> oe;
      oe = visitor.getRequest().get(PMC.SYS_HEADER);
      if (oe.isPresent()) {
        writeMap(xsw, "sys-header", (M) oe.get());
      }

      oe = visitor.getRequest().get(PMC.APP_HEADER);
      if (oe.isPresent()) {
        writeMap(xsw, "app-header", (M) oe.get());
      }

      oe = visitor.getRequest().get(PMC.LOCAL_HEADER);
      if (oe.isPresent()) {
        writeMap(xsw, "local-header", (M) oe.get());
      }
      
      oe = visitor.getRequest().get(PMC.BODY);
      if (oe.isPresent()) {
